...
def testaCurtirPostagemNovamente():
    ...
    with pytest.raises(Exception) as exc:
        curtir_postagem(2, 1)
    assert str(exc.value) == "Usuário já curtiu esta postagem."
    assert postagens[0]['curtidores'] == [2]
...
def test_curtir_postagem_inexistente():
    ...
    assert str(exc.value) == "Postagem inexistente."
...
def test_comentar_texto_em_branco():
    ...
    assert str(exc.value) == "Texto do comentário não pode ser vazio."
