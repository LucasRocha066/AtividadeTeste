...
def seguir_usuario(usuario_seguidor, usuario_a_seguir):
    ...
    usuario_seguidor_obj = encontrar_usuario_por_id(usuario_seguidor)
    usuario_a_seguir_obj = encontrar_usuario_por_id(usuario_a_seguir)
    usuario_seguidor_obj['seguindo'].append(usuario_a_seguir)
    usuario_a_seguir_obj['seguidores'].append(usuario_seguidor)

def curtir_postagem(usuario, postagem_id):
    if postagem_id <= 0 or postagem_id > len(postagens):
        raise IndexError("Postagem inexistente.")
    
    postagem_obj = encontrar_postagem_por_id(postagem_id)
...
def comentar_postagem(usuario, postagem_id, texto):
    if postagem_id <= 0 or postagem_id > len(postagens):
        raise IndexError("Postagem inexistente.")
